...
- オーディオクリップはキューに追加され、順番に再生されます (Sink キュー)
- `interrupt=true` にすると、新しい音声の再生前にキューがクリアされます
- 音量は設定から実行時に変更可能です

## アーキテクチャ

オーディオコマンド用の `mpsc` チャネルを持つ専用 OS スレッドを使用します。これは `rodio::OutputStream` が `!Send + !Sync` であるために必要です。エージェントは `AudioCommand` メッセージ (Play, SetVolume, Clear, Shutdown) を通じてオーディオスレッドと通信します。
